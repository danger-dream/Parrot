...
| 03 | [database](docs/03-database.md) | 当前 JSON StateStore、旧 `state.db` 迁移与独立 SQLite 库 |
...
6. **配置单源** — 所有可变配置集中在 `config.json`；运行时状态由 `runtime-cache.json` / `durable-state.json` 统一承载，`state.db` 仅作旧版本只读迁移源；业务日志按月分库在 `logs/`。
...
- httpx（上游客户端，连接池）
- xxhash（CCH 签名）
- JSON StateStore（运行状态）；sqlite3（按月日志、图片日志、OpenAI Store 等独立数据，按各自策略使用 WAL）
- 原生 stdlib 实现 TG 长轮询（httpx.Client）

## 监听与部署

- 默认监听端口 `22122`；已有 `config.json` 中显式配置的其他端口保持不变
