...
- **金额统计**：根据 LiteLLM 模型价格表，按输入 / 输出 / 缓存写入 / 缓存读取 Token 估算 USD，并在 Telegram 中所有展示 Token 缓存的请求、账户、渠道、模型和 API Key 位置同步显示金额；xAI OAuth 若返回 `cost_in_usd_ticks` 则优先采用上游真实金额。界面统一保留两位小数且不加约等号，未知模型明确显示“未计价”，不会静默按 `$0` 处理。价格源和 xAI 实际费用都以 USD 计价，因此不做实时汇率换算。
