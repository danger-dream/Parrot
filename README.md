...
- **金额统计**：根据 LiteLLM 模型价格表，按输入 / 输出 / 缓存写入 / 缓存读取 Token 估算 USD，并在 Telegram 中所有展示 Token 缓存的请求、账户、渠道、模型和 API Key 位置同步显示金额；支持目录中的长上下文阶梯价、OpenAI Priority 和 Anthropic Fast 倍率，长上下文按单次请求的全部 prompt Token 判断，多个短请求不会因聚合后超过阈值而被加价。xAI OAuth 若返回 `cost_in_usd_ticks` 则优先采用上游真实金额；界面会区分实际、估算和混合金额。金额统一保留两位小数且不加约等号，未知模型、无法确认 Token 口径的旧日志，以及无法从现有日志判断 5 分钟 / 1 小时 TTL 的 Anthropic 缓存写入请求都会明确显示“未计价”，不会静默按 `$0` 或错误档位处理。价格源和 xAI 实际费用都以 USD 计价，因此不做实时汇率换算。
