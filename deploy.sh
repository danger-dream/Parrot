...
PYEDIT
    ...
    if [[ -f "$cfg" ]]; then
        ...
        python3 - "$cfg" "$INSTALL_DIR" "${detected_cname:-parrot}" "${detected_svc:-anthropic-proxy}" "${detected_cport:-22122}" <<'PYEDIT'
import sys, json
import os
path, install_dir = sys.argv[1], sys.argv[2]
det_cname = sys.argv[3] if len(sys.argv) > 3 else "parrot"
det_svc = sys.argv[4] if len(sys.argv) > 4 else "anthropic-proxy"
