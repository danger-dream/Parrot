# 05 — Claude Code v2.1.280 mimicry

`src/transform/cc_mimicry.py` 是 Parrot 的 Claude Code（CC）messages 伪装实现。当前协议基线来自 v2.1.280 的本地 wire 语料与定稿验证器；旧 `cc-proxy` 只代表历史实现，**不再是 v2.1.280 oracle**。
...
## 5.2 v2.1.280 常量与 fingerprint

- `CC_VERSION = "2.1.280"`
- `CC_ENTRYPOINT = "sdk-cli"`
- UA：`claude-cli/2.1.280 (external, sdk-cli)`
...
索引遵循 JavaScript UTF-16 code unit，而不是 Python Unicode code point。若选中 surrogate 的单边，按 Node/Bun UTF-8 行为以 U+FFFD 参与 SHA-256；越界使用字符 `"0"`。emoji 固定向量 `ab🚀d🚀f🚀hijklmnopqr🚀t` 的结果为 `bc3`。
...
5. `fallback_credit_token` 及其余字段原样参与。
...
v2.1.280 的 8 个语义命名代表样本中，6 个带 CCH 的样本全部匹配；本地定稿验证器扫描当前全部抓包为 1688 ok / 0 fail。
...
Opus 4.8 与 Opus 5 的 v2.1.280 profile 默认都不带 `context-1m` 或 advisor；Opus 5 stdin 样本保留 fallback-credit。下游显式 1M 信号与 Parrot 的有界 1M 降级逻辑仍保留。
...
应用层顺序与 v2.1.280 语料一致：Accept、auth（OAuth 时）、Content-Type、UA、session、Stainless、beta、dangerous-direct-browser、version、API key（API-key 时）、x-app、request-class（main 时）、x-client-request-id。
...
main billing 顺序固定为 version → entrypoint → cch → workload → is_subagent → prev_req → prompt_id → `cc_turn_origin=sdk`。当前只有 prompt ID 有 request-scoped 权威来源；workload/subagent/prev_req 不凭空推断。side query 不带 prompt ID。
...
`test_cc_v2_1_280_upgrade.py` 覆盖：8 个真实代表样本、CCH 排除矩阵、220KB Fable、Fable/Opus/Haiku/side/OAuth profiles、OAuth scope/刷新/UA、auth、header 顺序、private-field stripping 与非 CC 边界。
