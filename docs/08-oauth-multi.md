...
`GET /v1/models` 是第四个独立边界：公开发现中的 Claude、OpenAI、xAI、Antigravity OAuth 模型由 Bot 的默认列表选择，分别读取 `oauthDefaultModels`、`openaiOAuth.defaultModels`、`xaiOAuth.defaultModels`、`antigravityOAuth.defaultModels`（普通 `list[str]`）。每个 Provider 只贡献默认列表与其启用且无 `disabled_reason` 的渠道目录的交集，并由同一渠道的 `supports_model` 确认实际支持；账户停用模型仍被排除。显式清空默认列表不会回退到内置列表或全部账户目录，缺失配置仍按现有 config 正规化补齐。API 渠道和 Cursor 保留原目录，同 ID 若有它们的合法路由不会被 OAuth 过滤误删；最终稳定去重排序。

此端点仍需 API Key 验证并应用 `allowedModels`；仅额外列出 global `model_mapping` aliases，真实目标必须在过滤后的目录中可见，Key 有白名单时别名和目标都必须获准。legacy ingress 专属别名不混入；不解析、不返回 account/models.dev metadata，binding 不能新增 ID，响应 schema 不扩展。默认列表经 Bot 保存或 config 热更新后立即生效，无额外缓存。这个 discovery-only 选择不修改账户 `models` / `account_model_catalog`、共享 registry 内部目录、调度或 `supports_model`：已有账户目录中的非默认模型仍可显式请求（遵循原路由与权限限制）。
