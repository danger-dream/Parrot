...
OAuth 备用模型已退役；旧 `/oauth_defaults`、`odm:*` 按钮与输入状态只显示退役提示，不写入配置。首次无账户目录时没有可路由模型，请同步上游；临时失败继续使用最后成功目录。
...
以下为公共配置领域，当前菜单位置见 [模型中心](14-model-center.md)。OAuth 备用模型入口与配置已完整退役：
...
OAuth「⚙️ 账户设置」保留 CCH 模式、用量显示方式、黑白进度条和配额监控等非模型设置，不再重复承载模型目录、备用模型或媒体配置。OAuth 备用模型已完整退役；图片和视频设置由模型中心承载；账户详情的「管理模型」则直接进入预选该账户来源的模型列表。OAuth 刷新、用量、官方重置卡、并发、清故障、亲和、启停、删除、登录 / 导入等既有能力继续保留。
...
时间范围首次没有缓存时，点击会显示加载页，后台完成后自动更新，无需重复点击；今日/本月尚未预热时也适用。用户冷缓存查询优先于下一项后台统计任务，但不会打断已运行的查询。重复点击共享同一查询，切换范围、维度或离开页面后，旧结果不会覆盖新页面。查询失败会显示错误，并保留刷新和范围切换按钮。

已有快照（包括过期快照）仍立即显示；保留原有后台刷新周期和统计口径，不增加全范围定时预热。
...
当名称/模型/email 含冒号或长度超标时，callback_data 中使用 hash 短码（4 字节），服务端用一个短码表映射回实际名。避免直接拼名称导致截断或歧义。旧 `oam:*`、Cursor 模型、`map:*`、`img:*`、`xim:*` callback 由兼容层安全重定向到当前模型中心对应页，不重放旧写意图；OAM 列表格式和带 modelRef / 勾选序号的单项格式分别解析，保留原 accountPage / filter，不能可靠解析时明确提示过期；`/oauth_defaults` 的退役提示先于宽 `/oauth`，旧 `odm:*` 不能重放写入。
