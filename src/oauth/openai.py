...
def _account_check_candidate(account: dict, *, account_id: str = "") -> dict:
    ...
    workspace_routing = codex_workspace_routing_patch(account)
...
def fetch_accounts_check_sync(access_token: str, *, org_id: str | None = None,
    ...
    try:
        ...
        # Optional metadata must not discard a successfully rotated token pair.
        return None
...
def enrich_token_response_sync(data: dict, *, workspace_id: str | None = None,
    ...
    for key in (
        ...
        if val:
            enriched[key] = val
    enriched.update(codex_workspace_routing_patch(info))
    return enriched
