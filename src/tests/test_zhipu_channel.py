...
from src.oauth.zhipu import signing
...
async def test_six_real_ingress_paths(env, ingress, stream):
    def wire(req):
        ...
        assert req.headers["user-agent"] == "ZCode/3.14.3"
        assert all(req.headers.get(k) for k in ("x-session-id", "x-request-id", "x-zcode-trace-id"))
...
async def test_production_http_hook_raw401_before_business_wrapper(env, monkeypatch):
    ...
    for key in ("x-request-id", "x-session-id", "x-zcode-trace-id"):
